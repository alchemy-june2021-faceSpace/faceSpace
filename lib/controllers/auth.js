...
async function fetchGoogleUser(id_token, access_token){
  await fetch(`https://www.googleapis.com/oauth2/v1/userinfo?alt=json&access_token=${access_token}`,
    {
      headers: {
...
module.exports = Router()
  ...
  .get('/login', async (req, res, next) => {
    try {
      ...
      console.log('AT / LOGIN', googleAuthURL);
  ...
  .get('/login/callback', async (req, res, next) => {
    ...
    await fetchGoogleUser(id_token, access_token);
