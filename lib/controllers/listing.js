...
module.exports = Router()
  ...
  .get('/price-high-to-low', async (req, res, next) => {
    try {
      const organize = await Listing.priceHighToLow();
      res.send(organize);
    } catch (error) {
      next(error);
    }
  ...
  .put('/:id', ensureAuth, async (req, res, next) => {
    try {
      const id = req.params.id;
      const userEmail = req.user.email;
      const putListing = await ListingService.updateListing(
        id,
        userEmail,
        req.body
      );
  ...
  .delete('/:id', ensureAuth, async (req, res, next) => {
    try {
      const id = req.params.id;
      const userEmail = req.user.email;
      const deleteListing = await ListingService.removeListing(
        id,
        userEmail,
        req.body
      );
